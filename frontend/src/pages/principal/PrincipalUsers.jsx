...
export default function PrincipalUsers() {
  ...
  const fetchRequests = async () => {
    ...
    try {
      const res = await api.get("/api/principal/requests");
...
const approveRequest = async (id) => {
  ...
  try {
    await api.post(`/api/principal/requests/${id}/approve`);

    setOk("Request approved and user created ✅");

    fetchRequests();
    fetchUsers(); // refresh user list to see new user
  } catch (e) {
    setErr(e?.response?.data?.message || "Failed to approve request");
  }
};
  ...
  const rejectRequest = async (id) => {
    ...
    try {
      await api.post(`/api/principal/requests/${id}/reject`);
      fetchRequests();
    } catch (e) {
      console.error(e);
    }
    ...
    <div className="dashboard-container">
      ...
      <div className="pu-container">
        ...
        <div className="pu-grid">
                    ...
                    {rows.map((u) => (
                      <Fragment key={u.id}>
                        <tr>
                          ...
                          <td>
                            <span className="pu-pill pu-role">{u.role}</span>
                          </td>
          ...
          {/* LEFT: Registration Requests */}
          <div className="pu-card">
            <div className="pu-card-title">Registration Requests</div>
                  ...
                  <tbody>
                    {requests.map((r) => (
                      <tr key={r.id}>
                        ...
                        <td style={{ textAlign: "right" }}>
                          <button
                            className="pu-btn pu-tiny pu-success"
                            onClick={() => approveRequest(r.id)}
                          >
                            Approve
                          </button>
                          <button
                            className="pu-btn pu-tiny pu-danger"
